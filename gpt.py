...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        out = torch.cat([h(x) for h in self.heads], dim=-1) # run all in parallel and concat all outputs, over channel dimension
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        tok_emb = self.token_embedding_table(idx) # (B, T, C) T = context size
